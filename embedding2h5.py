...
def export_data_h5(vocabulary, embedding_matrix, output='embedding.h5'):
    ...
    _dset = f.create_dataset('embedding', embedding_matrix.shape,
                             dtype=embedding_matrix.dtype, **compress_option)
...
def fasttext_export(embedding_file):
    ...
    for line in fin:
        ...
        line_idx += 1
    export_data_h5(vocabulary, np.array(
        embeddings, dtype=np.float32), output=embedding_file + ".h5")
