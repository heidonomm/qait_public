...
class KGDQNTrainer(object):
    ...
    def __init__(self, params, games, games_dir):
        ...
        self.filename = 'kgdqn_' + \
            '_'.join([str(v)
                      for k, v in params.items() if 'file' not in str(k)])
        logging.basicConfig(filename='./kgdqn/logs/' +
        ...
        if self.use_cuda:
            ...
            self.model = KGDQN(params, self.state.all_actions)
        ...
        if params['scheduler_type'] == 'exponential':
            self.e_scheduler = ExponentialSchedule(
                self.num_frames, params['e_decay'], params['e_final'])
        elif params['scheduler_type'] == 'linear':
            self.e_scheduler = LinearSchedule(
                self.num_frames, params['e_final'])

    def plot(self, frame_idx, rewards, losses, completion_steps):
        fig = plt.figure(figsize=(20, 5))
        plt.subplot(131)
        plt.title('frame %s. avg reward: %s' %
                  (frame_idx, np.mean(rewards[-10:])))
        plt.plot(rewards)
        plt.subplot(132)
        plt.title('frame %s. avg steps: %s' %
        ...
        fig.savefig('./kgdqn/plots/' + self.filename +
                    '_' + str(frame_idx) + '.png')
        # plt.show()
    ...
    def compute_td_loss(self):
        ...
        with torch.no_grad():
            ...
            fwd_init, sts = self.model.forward_td_init(
                next_state, actions[:, 0, :])  # .unsqueeze_(0)
    ...
    def train(self):
        ...
        for e_idx in range(1, self.num_episodes + 1):
            ...
            for frame_idx in range(1, self.num_frames + 1):
                ...
                state = self.state
                self.state.step(
                    next_state.description, prev_action=prev_action, pruned=self.params['pruned'])
                prev_action = action_text
                self.replay_buffer.push(
                    state, action, reward, self.state, done)
        ...
        torch.save(parameters, './kgdqn/models/' + self.filename + '_final.pt')
        self.env.close()
