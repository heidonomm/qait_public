...
class GraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=False):
        ...
        self.W = nn.Parameter(nn.init.xavier_uniform_(torch.Tensor(in_features, out_features).type(
            torch.cuda.FloatTensor if torch.cuda.is_available() else torch.FloatTensor), gain=np.sqrt(2.0)), requires_grad=True)
...
class EncoderLSTM(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_size, hidden_size, padding_idx,
        ...
        # nn.Embedding(vocab_size, embedding_size, padding_idx)
    ...
    def _forward_unpadded(self, x, x_mask):
        ...
        if self.dropout_output and self.dropout_rate > 0:
            output = F.dropout(output,
                               p=self.dropout_rate,
                               training=self.training)
            hst_opt = F.dropout(hst_opt, p=self.dropout_rate,
                                training=self.training)
